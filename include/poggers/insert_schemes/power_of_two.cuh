...
struct __attribute__ ((__packed__)) power_of_two_inserts {


	//tag bits change based on the #of bytes allocated per block

	static_assert(Max_Probes == 2, "Power of two hashing must probe exactly twice\n");
...
public:
	...
	__host__ __device__ power_of_two_inserts(): num_buckets(0), seed(0) {};


	//only allowed to be defined on CPU
	__host__ power_of_two_inserts(Internal_Rep<Key, Val> * ext_slots, uint64_t ext_nslots, uint64_t ext_seed): num_buckets(ext_nslots), seed(ext_seed){
