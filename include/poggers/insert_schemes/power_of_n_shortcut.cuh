...
struct __attribute__ ((__packed__)) power_of_n_insert_shortcut_scheme {
...
public:
	...
	using my_type = power_of_n_insert_shortcut_scheme<Key, Val, Partition_Size, Bucket_Size, Internal_Rep, Max_Probes, Hasher, Probing_Scheme>;
	...
	__host__ __device__ power_of_n_insert_shortcut_scheme(): num_buckets(0), seed(0) {};


	//only allowed to be defined on CPU
	__host__ power_of_n_insert_shortcut_scheme(Internal_Rep<Key, Val> * ext_slots, uint64_t ext_nslots, uint64_t ext_seed): num_buckets(ext_nslots), seed(ext_seed){
