...
__global__ void malloc_test_kernel(bitarr_grouped<4> * global_bitarray, uint64_t max_mallocs, uint64_t mallocs_per_thread, uint64_t max_blocks){
...
__global__ void malloc_test_kernel_group(bitarr_grouped<4> * global_bitarray, uint64_t max_mallocs, uint64_t mallocs_per_thread, uint64_t max_blocks){
...
__host__ void build_bitarr_test(uint64_t max_mallocs, uint64_t mallocs_per_thread, uint64_t block_size){
   ...
   malloc_test_kernel_group<<<(max_mallocs -1)/block_size+1, block_size>>>(dev_bitarray, max_mallocs, mallocs_per_thread, max_blocks);
...
int main(int argc, char** argv) {
   ...
   build_bitarr_test_split(1024, 1, 512);


   build_bitarr_test_split(2048, 1, 512);



   build_bitarr_test_split(4096, 1, 512);


   build_bitarr_test_split(8192, 1, 512);



   build_bitarr_test_split(8192*2, 1, 512);
