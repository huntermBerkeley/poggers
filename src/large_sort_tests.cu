...
__host__ uint64_t * generate_data(uint64_t nitems){
	...
	//			   100,000,000
	const uint64_t cap = 100000000ULL;

	//const uint64_t cap = 100000ULL;
	...
	for (to_fill =0; to_fill < nitems; to_fill+=cap){



		uint64_t togen = (nitems - to_fill > cap) ? cap : nitems - to_fill;

		printf("%d Starting at %llu/%llu, adding %llu items to get to %llu\n", omp_get_thread_num(), to_fill, nitems, togen, to_fill+togen);
		fflush(stdout);
	...
	// #pragma omp parallel for
	// for (uint64_t to_fill = 0; to_fill < nitems; to_fill+=cap){
	...
	// 	printf("%d Generated %llu/%llu\n", omp_get_thread_num(), to_fill, nitems);
	// 	fflush(stdout);
	...
	auto end = std::chrono::high_resolution_clock::now();
  	...
  	std::cout << "Generated " << nitems << " in " << diff.count() << " seconds\n";
...
int main(int argc, char** argv) {
	...
	printf("max threads available: %llu\n", omp_get_max_threads());
