...
__host__ void bulk_query_fp_timing(optimized_vqf* my_vqf, uint64_t * vals, uint64_t nvals, uint64_t * misses){
...
int main(int argc, char** argv) {
	...
	for (int batch = 0; batch< num_batches; batch++){
		...
		//launch inserts
		diff += insert_timing(my_vqf, dev_vals, items_to_insert, misses);
		...
		//launch queries
		bulk_query_timing(my_vqf, dev_vals, items_to_insert, misses);
		...
		//false queries
		bulk_query_fp_timing(my_vqf, dev_vals, items_to_insert, misses);
